...
- 已登录用户读取公开课题列表、课题详情页、课题进度页和状态卡时，每个课题应带 `viewer_state`，用于从数据库恢复收藏、点赞、参与、认领、资助等按钮状态；已撤回关系不能继续算作当前激活状态。`viewer_state.team_role_keys` 必须只包含当前用户 active 参与或项目负责人认领命中的组队角色 key（`doctor/student/mentor/leader`），论文第一单位认领和资助不进入组队角色高亮；`viewer_state.sponsor_requests` 必须包含当前用户 active 资助请求的 `id/sponsor_type/status` 等明细，支持从详情页深链直接撤回 token 等二级资助类型。
...
- 匿名访客公开列表、详情、状态卡只展示人员 UID，不展示用户名、昵称、真实姓名、邮箱或微信；登录态课题列表还可返回创建者和组队角色联系人姓名/微信，但联系方式展示范围属于独立隐私策略，未产品确认前不新增 `contact_visibility` 字段。
...
- 管理员申请审批列表默认 `pending` 优先，待处理申请按创建时间正序，已处理申请按更新时间倒序；资助多选在管理端按同一用户同一课题聚合展示，但单项审批仍逐条落库和审计。
