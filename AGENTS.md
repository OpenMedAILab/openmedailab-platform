...
| 管理员工作台、申请审批、任务管理、系统入口、备份恢复 | 2.6、3.5、4.6、6.3 |
...
- 新注册用户初始积分按身份发放：医生 250 分，博士在读和博士毕业及以上 200 分，学生及其他身份 100 分；历史用户积分不自动回填。
...
- 已登录用户读取公开课题列表、课题详情页、课题进度页和状态卡时，每个课题应带 `viewer_state`，用于从数据库恢复收藏、点赞、参与、认领、资助等按钮状态；已撤回关系不能继续算作当前激活状态。`viewer_state.team_role_keys` 必须只包含当前用户 active 参与或项目负责人认领命中的组队角色 key（`doctor/student/mentor/leader`），论文第一单位认领和资助不进入组队角色高亮；`viewer_state.sponsor_requests` 必须包含当前用户 active 资助请求的 `id/sponsor_type/status` 等明细，供算力和劳务两个资助入口分别恢复状态。
- 公开课题列表默认使用 `sort=recommended`：登录用户先按本人相关课题置顶，再按 `点赞数 + 关注数` 降序；访客直接按 `点赞数 + 关注数` 降序。用户切换“编号顺序”时才按 `topic_id` 正序，“最新编号”按 `topic_id` 倒序，“最近更新”按 `updated_at` 排序；`sort=likes` 必须由后端在分页前按 `score_count` 排序。
...
- 状态卡展示阶段、我的关系、按收藏/参与/认领/资助分组的 UID；`uid_groups.groups` 必须返回结构化 `type/status/subtype/subtype_label`，前端不得靠中文 label 推断状态。
- 匿名访客公开列表、详情、状态卡只展示人员 UID，不展示用户名、昵称、真实姓名、邮箱或微信；登录态课题列表可返回创建者和组队角色联系人姓名/微信，资助悬浮卡可展示已通过资助者 UID 和微信，但不得展示资助者用户名、昵称、真实姓名、邮箱或审核信息。
- 课题卡片组队只展示 `0/1`、`1/1`、`2/1` 这类计数文字；超过目标人数时不追加文字提示，也不使用单独的超额配色，按普通已满足角色视觉展示；如果该角色命中当前用户的 `viewer_state.team_role_keys`，在卡片和进度页都应有金色“我的角色”强调。
- 课题标题进入 `#/project/{id}` 独立项目进度页；`#/project/{id}/progress` 作为同一进度页别名；主 PDF 仍通过单独查看/下载入口打开。
- 项目进度页只展示公开且未归档课题、公开进度文档和公开进度记录，不再嵌入讨论区。
- 讨论区是独立功能页 `#/project/{id}/discussion`：访客可读，登录用户可写主讨论和回复；作者可编辑/删除自己的讨论，管理员可隐藏/恢复/删除，讨论写操作必须写审计。
...
| 前端操作 | `frontend/src/main.js` | `submitLike`、`submitInterest`、`submitParticipationRequest`、`submitClaim`、`submitLeadClaim`、`submitPaperClaim`、`openSponsorModal`、`withdrawInteraction` |
...
- 点赞是轻量反馈，不推进课题生命周期。
- 平台管理员不能参与、认领、资助任何课题，也不能提交任务结果；管理员历史协作记录不计入当前组队、资助统计、状态卡 UID 分组、本人协作状态或任务结果提交资格。收藏和点赞仍属于轻量反馈，不视为参与项目。
...
- 无法认领时，前端必须展示鼠标/焦点附近的小提示或移动端 toast，原因来自 `claim_availability`，覆盖未登录、阶段原因、已有认领、身份并行课题上限、积分不足和数据冲突。
...
- 课题卡和详情页资助主入口拆为两个与“参与”同级的独立按钮：`资助算力` 和 `资助劳务`。每个按钮只处理自己的 `sponsor_type`；没有 active 资助时点击打开小弹窗填写资助金额/算力情况，已有 active 资助时按钮文案分别显示 `管理资助算力`、`管理资助劳务`，弹窗内可追加说明或用明确按钮撤回对应类型。前端主流程不得再使用 `管理资助`、多选 checkbox、取消勾选后提交的隐式撤回模式或 `更多资助类型` 作为卡片/详情主入口。
- 管理员在课题卡、课题详情和个人相关课题列表中应看到与普通用户一致的协作按钮布局；参与、认领、`资助算力` 和 `资助劳务` 不能被直接隐藏。管理员资助按钮必须灰态禁用，悬停或聚焦时提示 `系统管理员不能资助项目`，点击时不能打开资助弹窗。
- 登录用户悬停或聚焦 `资助算力`、`资助劳务` 按钮时，若该类型已有 approved 资助，展示与组队按钮一致风格的浮窗；每个资助者上方显示 UID，下方显示资助者微信，未填写微信时显示未填写状态。管理员悬停或聚焦资助按钮时展示不能资助的提示；资助悬浮卡不得展示资助者用户名、昵称、真实姓名、邮箱或审核信息。
- 历史 `token` 等非主入口资助类型保留 API、个人中心和管理端兼容展示；active 历史记录可通过个人中心行操作撤回，但卡片和详情页不提供 token 提交或重新提交入口。
...
- 参与和认领提交前必须先检查身份并行课题硬上限，再检查可用积分额度；医生最多同时参与/认领 5 个非归档课题，博士在读和博士毕业及以上最多 4 个，学生及其他身份最多 2 个，该上限不随转赠或奖励积分增加。达到上限返回稳定错误码 `participation_limit_reached`。
- 可用积分额度规则：余额扣除未启动但已占用的 50 分参与额度后，仍需至少剩余 50 分。
- 登录用户公开课题库的“默认”排序按本人关系优先展示：后端必须在分页前按 `我已认领`、`我已参与`、`我已资助`、无关系的优先级排序，再按 `点赞数 + 关注数` 排序；用户切换其他排序方式后不再强制本人关系置顶。多关系时首页课题库卡片只显示一个金色斜角主标签，其余本人关系以金色小标签显示在课题编号前。本人相关课题 hover 顶线为金色。
...
- 积分自动运营规则属于主流程：注册初始积分按身份发放（医生 250 分、博士在读/博士毕业及以上 200 分、学生及其他身份 100 分）；首次完善资料奖励 5 分；项目启动扣除每名参与人 50 分；进行中课题归档视为结题，返还每名参与人 100 分；用户可单次最多转赠 50 分。并行课题数由身份硬上限控制，不由余额推导。

### 2.6 管理员工作台、申请审批、任务管理、系统入口、备份恢复、用户管理
...
- 管理员不审核普通参与；管理员通过 `申请审批` 审批认领项目负责人、认领课题（论文第一单位）和资助意向，并填写可返回给申请人的审核意见；不要把该入口命名为 `任务审批`，避免与任务结果管理混淆。
- 管理员申请审批列表默认 `pending` 优先，待处理申请按创建时间正序，已处理申请按更新时间倒序；同一用户同一课题下的多个资助项在管理端聚合展示，但单项审批仍逐条落库和审计。
...
- 课题主 PDF、项目进度 PDF、主题数据集说明 PDF 和任务结果 PDF 都必须校验扩展名、20MB 大小上限和 `%PDF-` 文件签名；非法上传返回稳定错误并写 failed audit。
...
- 课题卡片中组队未满足、已满足、本人角色、进行中、已获资助使用不同语义颜色；超过目标人数不使用单独配色，按已满足角色视觉展示，本人角色使用金色强调且不能替代必要计数。
...
| 平台管理员 | 管理全站用户、主题、数据集说明 PDF、课题、项目负责人/论文第一单位认领审批、资助审批、任务结果查看、审计日志、密码恢复、收藏和点赞公开课题 | 参与、认领、资助或提交任务结果；通过前端创建第二个管理员、审批普通参与或历史兼容认领类型、审核任务结果、恢复邮箱找回密码主流程 |
...
- 积分主规则：注册初始积分按身份发放，医生 250 分、博士在读/博士毕业及以上 200 分、学生及其他身份 100 分；首次完善资料 +5；项目启动扣除每名参与人 50；进行中课题归档返还每名参与人 100；单次转赠上限 50；并行课题硬上限为医生 5 个、博士在读/博士毕业及以上 4 个、学生及其他身份 2 个。
...
平台管理员不得发起参与、认领、资助或任务结果提交；历史管理员协作行保留但不进入当前关系统计和状态展示。
...
| `POST` | `/api/projects/{id}/interest/` | 参与，自动通过；必须提交署名意向；平台管理员返回 `platform_admin_cannot_collaborate` |
| `POST` | `/api/projects/{id}/claim/` | 认领；项目负责人和论文第一单位待管理员审批，论文第一单位必须提交 `claimed_unit_name`，其他兼容认领自动通过；平台管理员返回 `platform_admin_cannot_collaborate` |
| `POST` | `/api/projects/{id}/sponsor/` | 资助意向，待审批；平台管理员返回 `platform_admin_cannot_collaborate` |
...
- `uid`: 平台唯一 UID
- `real_name`: 对外显示姓名；前端表单文案为“对外显示姓名”，数据库字段名保持 `real_name` 兼容历史。
- `role_type`: 用户身份
- `contact_email` / `email_normalized`: 联系邮箱和唯一性约束
- `must_change_password`: 默认密码登录后的强制改密标记
- `last_seen_at`: 最近一次登录用户请求活动时间，仅用于在线人数聚合统计
- `credit_balance`: 当前积分余额，普通用户可在个人中心总览查看；新注册用户默认值由 `initial_credits_for_role()` 按身份计算。
...
- 资助意向待管理员审批。
- 卡片和详情页主流程只开放算力和劳务两个资助类型；其他历史资助类型仅做兼容展示和撤回。
...
- 我的申请：参与、认领和资助申请状态、审核意见和撤回/重新提交入口。
...
- 总览。
- 申请审批：处理项目负责人认领、论文第一单位认领和资助意向；拒绝必须填写审核意见。
...
7. 用户登录、收藏、取消收藏。
8. 用户参与、认领、分别点击 `资助算力` 和 `资助劳务`，观察状态反馈、独立管理文案和已资助者 UID/微信悬浮卡。
9. 用户进入个人中心，查看我的任务、我的申请、我上传、个人资料和当前积分。
10. 用户提交任务结果，可同时上传 PDF 或 Markdown 文档。
11. 管理员登录，进入申请审批、任务管理、课题管理、主题管理、系统入口、备份恢复、用户管理、审计日志。
12. 管理员审批认领和资助；拒绝时验证审核意见必填；查看任务结果文档、修改课题阶段。
...
- 平台管理员不能参与、认领、资助或提交任务结果；`/api/rbac/` 中管理员的 `express_interest`、`claim_work`、`sponsor_project` 必须为 `false`。
- 本人项目负责人/论文第一单位认领 pending 时显示审批中，approved 后才显示具体撤回入口。
- 课题卡和详情页必须使用 `资助算力`、`资助劳务` 两个独立按钮，已有 active 资助时分别显示 `管理资助算力`、`管理资助劳务`；管理员也必须看到这两个按钮，但按钮灰态禁用并悬浮提示 `系统管理员不能资助项目`；主流程不得出现多选资助、`更多资助类型` 或泛化 `管理资助` 入口。
- 资助由管理员审批；已获批资助悬浮卡按对应类型展示资助者 UID 和微信，不展示姓名、邮箱或审核信息。
- 管理端认领/资助审批入口必须叫 `申请审批`；拒绝 claim/sponsor 必须填写审核意见。
- 个人中心 `我的申请` 必须展示 pending/approved/rejected/withdrawn 状态和审核意见。
- 非进行中课题不能提交任务结果。
- 未参与/未认领用户不能提交任务结果。
...
- 资助统计只统计 approved；审核意见要在个人中心和管理端一致可见。
- `status-card.uid_groups.groups` 必须使用结构化 `type/status/subtype/subtype_label`。
- 点赞排序必须在后端分页前完成。
- 匿名访客只能看到人员 UID。
- API 字段错误必须双写 `error.details` 和顶层 `errors`。
- PDF 上传必须校验签名和大小。
- Playwright E2E 与 `scripts/qa_full.sh` 是最终验收门禁。
- 团队满足且已获资助不会自动改变课题阶段。
- 所有写操作有审计。
- 二维码只能由管理员上传或更新；公开查看不写审计，上传成功写 `platform_qr.upload` 审计，未上传的二维码不公开渲染。
- 所有失败有明确反馈。
- 涉及人员展示时默认只展示 UID；只有已明确允许的登录态联系人场景才展示微信，例如组队悬浮卡和已通过资助者悬浮卡。
...
- 是否避免新增冗余 API？
- 是否避免恢复旧任务拆分和积分主流程？
- 是否按场景白名单展示人员信息，避免在未授权位置暴露姓名、邮箱、微信或审核信息？
- 是否补齐权限校验、事务和审计？
- 是否确认内容备份恢复的范围、权限和审计？
- 是否补齐明确错误和前端反馈？
- 是否处理小屏和弹窗层级？
...
- 是否跑了对应测试？
- 是否运行 `./scripts/qa_full.sh`，或至少明确说明其中哪些命令因环境限制未执行？
- 是否做了 Playwright 浏览器验收，覆盖 `1440x1000`、`768x1000`、`390x844`、`360x740`、`320x740`？
