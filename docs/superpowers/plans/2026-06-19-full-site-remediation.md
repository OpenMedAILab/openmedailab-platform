...
- [x] **Step 5: 实现模型、admin 和 serializer**
...
- [x] **Step 4: 给 modal 模板接入 ref 和 keydown**
...
- [x] **Step 2: 联系人 hover card 改 viewport 安全**
...
- [x] **Step 2: 重构 admin_project_update 事务**
...
- [x] **Step 2: ErrorEnvelope schema 和 error_payload 双写**
...
执行结论：本计划已完成代码实现、文档治理、自动化验收和多 agent 复审。Task 2 的联系方式展示范围按产品门禁处理：本轮只收紧匿名隐私泄露，未新增 `contact_visibility` 模型字段或迁移；该条件分支在未获产品确认时视为门禁通过。

### 7.1 Checklist 状态

- 本文件 82 个执行 checklist 已全部标记为完成。
- 条件项均按计划中的门禁条件处理，未触发的模型/迁移分支未实现，也未作为当前产品规则发布。

### 7.2 最终 QA 证据

完整命令输出已保存到 `docs/qa/2026-06-19-full-site-remediation-qa.log`。

最终通过摘要：

- `.venv/bin/python manage.py check`：通过。
- `.venv/bin/python manage.py makemigrations --check --dry-run`：无迁移变更。
- `.venv/bin/python manage.py test accounts api projects interactions credits`：107 个测试通过。
- `node --test frontend/src/*.test.js`：69 个测试通过。
- `npm run build`：通过。
- `npm run test:e2e`：45 个 Playwright 测试通过，覆盖 `1440x1000`、`768x1000`、`360x740`、`390x844`、`320x740`。
- `git diff --check`：通过。
- `python scripts/check_release_version.py`：`release version ok: 0.14.0`。

### 7.3 最终复审记录

- 前端/QA reviewer：PASS，无阻塞项。
- 后端/API reviewer：初次复审发现 PDF 上限、oversized 上传测试、sponsor 重提测试、DB lock 确定性测试缺口；已补齐实现与测试，并通过全量 QA。
- 计划闭环 reviewer：初次复审发现 checklist、QA 证据、PM 审计归档、旧文档口径、归档确认一致性和 AGENTS 日期缺口；已全部修正，并通过全量 QA 与文档扫描。
