...
本文从资深产品经理视角，基于当前代码仓库已有模型、API 和前端页面，重新固定 OpenMedAILab 的课题生命周期。本文是后续实现和验收的产品依据；如旧文档仍包含“写作中、投稿中、已发表、任务拆分、任务分配、任务积分”等描述，以本文为准。
...
| `ProjectFollow` | `interactions/models.py` | 用户收藏课题 | 只作为用户关系，不作为课题状态 |
...
| 用户收藏/取消收藏 | `/api/projects/{id}/follow/` 和 `/unfollow/` | 保留 |
...
| `open_recruiting` | 开放招募 | 是 | 课题公开，允许用户收藏、申请参与、认领、资助 |
...
- 收藏表达“我关注这个课题”，不代表课题进入任何生命周期阶段。
- 收藏可以用于用户空间、课题卡片、排序信号和热度统计。
- 收藏不会触发 `open_recruiting -> team_building`。
- 收藏不会触发通知管理员审核。

结论：收藏只算用户与课题的关系，不算课题生命周期状态。
...
收藏是用户关系，因此只影响这个页面和课题卡片上的“我的状态”，不影响课题阶段。
...
| 草稿 | 用户不可见 |
| 开放招募 | 收藏、申请参与、认领、资助 |
| 组队中 | 收藏、申请参与、认领、资助、查看组队状态 |
| 进行中 | 收藏、提交任务结果；不允许新申请、认领和资助 |
...
课题状态应只表达课题本身从“未发布”到“招募、组队、进行、暂停、关闭”的进展。收藏只是用户对课题的个人关注，适合进入“我的状态”和热度统计，不适合推动课题生命周期。
