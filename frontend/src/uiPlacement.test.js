...
test("project cards keep compact summary UI without hover status styles", () => {
  ...
  assert.match(mainSource, /sponsorTypeRequestCanWithdraw\(project,\s*'labor_fee'\)/);
  assert.match(mainSource, /sponsorTypeRequestCanWithdraw\(project,\s*'compute'\)/);
  ...
  assert.match(mainSource, /sponsorTypeButtonLabel\(project,\s*'labor_fee'\)/);
  assert.match(mainSource, /sponsorTypeButtonLabel\(project,\s*'compute'\)/);
...
test("project lifecycle actions are gated by project stage and review status", () => {
  ...
  assert.match(mainSource, /async function handleSponsorAction\(project,\s*sponsorType\)/);
  assert.match(mainSource, /QUICK_SPONSOR_TYPES = \["labor_fee", "compute"\]/);
  assert.match(mainSource, /sponsorTypeButtonLabel\(project,\s*sponsorType\)/);
  assert.match(mainSource, /sponsorTypeRequestCanWithdraw\(project,\s*sponsorType\)/);
  assert.match(mainSource, /api\.sponsor\(project\.id,\s*\{\s*sponsor_type:\s*sponsorType/);
  assert.match(mainSource, /api\.withdrawInteraction\("sponsor",\s*activeRequest\.id/);
  assert.match(mainSource, /isSponsorTypeBusy\(project,\s*sponsorType\)/);
  assert.match(mainSource, /撤回\$\{typeLabel\}/);
  assert.doesNotMatch(mainSource, /v-model="state\.forms\.sponsor\.sponsor_types"/);
  assert.doesNotMatch(mainSource, /class="sponsor-popover"/);
  assert.doesNotMatch(mainSource, /aria-controls="sponsor-popover"/);
  assert.doesNotMatch(mainSource, /管理资助/);
  assert.doesNotMatch(mainSource, /当前资助/);
  assert.doesNotMatch(mainSource, /sponsor-current-list/);
  assert.doesNotMatch(mainSource, /sponsor-current-row/);
  assert.doesNotMatch(mainSource, /withdrawSponsorRequest\(state\.sponsorModal\.project,\s*request\)/);
  assert.match(mainSource, /confirmText:\s*`撤回\$\{typeLabel\}`/);
  assert.doesNotMatch(mainSource, /requestsToWithdraw/);
  assert.doesNotMatch(stylesSource, /\.sponsor-current-list\s*\{/);
  assert.doesNotMatch(stylesSource, /\.sponsor-current-row\s*\{/);
  assert.doesNotMatch(stylesSource, /\.sponsor-popover\s*\{/);
  ...
  assert.doesNotMatch(mainSource, /async function submitLeadClaim\(project\)[\s\S]*?await loadProjects\(\{ reset: true \}\);[\s\S]*?async function handleSponsorAction/);
  assert.doesNotMatch(mainSource, /async function handleSponsorAction\(project,\s*sponsorType\)[\s\S]*?await loadProjects\(\{ reset: true \}\);[\s\S]*?async function withdrawInteraction/);
  ...
  assert.doesNotMatch(mainSource, /SPONSOR_POPOVER_HEIGHT/);
  assert.doesNotMatch(mainSource, /function sponsorPopoverPositionFromTrigger/);
  assert.doesNotMatch(mainSource, /function realignSponsorPopoverFromRenderedSize/);
  assert.doesNotMatch(mainSource, /handleSponsorPopoverViewportChange/);
  ...
  assert.doesNotMatch(mainSource, /state\.forms\.sponsor/);
  ...
  assert.doesNotMatch(mainSource, /closeSponsorModal\(\)/);
...
test("project progress page exposes the same core interaction actions as project cards", () => {
  ...
  assert.match(mainSource, /handleSponsorAction\(state\.projectProgress\.project,\s*'labor_fee'\)/);
  assert.match(mainSource, /handleSponsorAction\(state\.projectProgress\.project,\s*'compute'\)/);
});
...
test("modal accessibility hooks keep focus inside dialogs and restore triggers", () => {
  ...
  assert.doesNotMatch(mainSource, /ref="sponsorModalRef"/);
...
test("visual e2e hooks and decorative icon semantics are present", () => {
  ...
  assert.doesNotMatch(mainSource, /data-testid="sponsor-popover"/);
...
test("sponsor actions expose labor fee and compute as independent buttons", () => {
  assert.match(mainSource, /const QUICK_SPONSOR_TYPES = \["labor_fee",\s*"compute"\]/);
  assert.match(mainSource, /handleSponsorAction\(project,\s*'labor_fee'\)/);
  assert.match(mainSource, /handleSponsorAction\(project,\s*'compute'\)/);
  assert.match(mainSource, /handleSponsorAction\(state\.projectProgress\.project,\s*'labor_fee'\)/);
  assert.match(mainSource, /handleSponsorAction\(state\.projectProgress\.project,\s*'compute'\)/);
  assert.match(mainSource, /sponsorTypeButtonLabel\(project,\s*'labor_fee'\)/);
  assert.match(mainSource, /sponsorTypeButtonLabel\(project,\s*'compute'\)/);
  ...
  assert.doesNotMatch(mainSource, /sponsorPopoverTypes/);
});
